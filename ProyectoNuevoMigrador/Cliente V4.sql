...
--Script para leer archivos de disco
--declare @t varchar(max) 
--exec ns_txt_file_read 'C:\APLICACIONES\FORMATOS\Config.ini', @t output 
----select @t as [ArchivoDeTexto.txt] 
----SELECT PATINDEX('%Estado =%',  @t)
----SELECT PATINDEX('%Empresa =%',  @t)
--DECLARE @flag varchar(max) = substring(@t,PATINDEX('%Estado =%',  @t)+9,PATINDEX('%Empresa = %',  @t)-(PATINDEX('%Estado =%',  @t)+9)) 
...
-- FETCH NEXT FROM CURSORBD
-- INTO @NombreTabla

-- WHILE @@FETCH_STATUS = 0
-- BEGIN
-- 	DECLARE @sentencia nvarchar(max) = 'select @Salida= count(*)  from '+@NombreTabla
-- 	DECLARE @Salida int
-- 	DECLARE @Parametros NVARCHAR(max)= N'@Salida int OUTPUT';
-- 	EXEC SP_EXECUTESQL @Sentencia, @Parametros, @Salida=@Salida OUTPUT
-- 	FETCH NEXT FROM CURSORBD
...
-- 	VALUES
-- 	(
-- 	    @NombreTabla, -- NombreTabla - VARCHAR
-- 	    @Salida -- TotalFilas - int
-- 	)
-- 	FETCH NEXT FROM CURSORBD
--      INTO @NombreTabla
-- END
...
-- 			 @Nom_Sucursal,'|' ,
-- 			 @Dir_Sucursal, '|' ,
-- 			 CONVERT(varchar,@Por_UtilidadMax), '|' ,
-- 			 CONVERT(varchar,@Por_UtilidadMin), '|' ,
-- 			 @Cod_UsuarioAdm, '|' ,
-- 			 @Cabecera_Pagina, '|' ,
-- 			 @Pie_Pagina, '|' ,
-- 			 CONVERT(varchar,@Flag_Activo), '|' ,
-- 			 @Cod_Ubigeo, '|' ,
-- 			 @Cod_UsuarioReg, '|' ,
-- 			 CONVERT(varchar,@Fecha_Reg,121), '|' ,
-- 			 @Cod_UsuarioAct, '|' ,
...
--Borran todos los triggers activos sobre las tablas
--Para pruebas si por alguna razon el responsable del area no se encuentra
...
-- WHERE  so .type = 'TR' 
-- AND so.name LIKE '%_IUD'


-- SELECT DISTINCT 'IF EXISTS (SELECT name  FROM   sysobjects WHERE  name ='+''''+
--    so.name+''''+' AND 	  type = '+''''+'TR'+''''+') DROP TRIGGER '+so.name
-- FROM   sys .objects so
-- INNER JOIN sys. sql_modules sm
-- ON so.object_id = sm.object_id
-- WHERE  so .type = 'TR' 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script='PALERPlink.'+@NombreBD+'.dbo.USP_ALM_ALMACEN_MOV_I '+ 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_ALM_ALMACEN_MOV_D_I '+ 
			  CASE WHEN M.Cod_TipoComprobante IS NULL THEN 'NULL,' ELSE ''''+ REPLACE(M.Cod_TipoComprobante,'''','') +''',' END +
			  CASE WHEN M.Serie IS NULL THEN 'NULL,' ELSE ''''+ REPLACE(M.Serie,'''','') +''',' END +
			  CASE WHEN M.Numero IS NULL THEN 'NULL,' ELSE ''''+ REPLACE(M.Numero,'''','') +''',' END + 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_BAN_CUENTA_BANCARIA_I '+
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_BAN_CUENTA_M_I '+ 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
				 SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_CAJ_CAJA_MOVIMIENTOS_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script='PALERPlink.'+@NombreBD+'.dbo.USP_CAJ_COMPROBANTE_D_I '+ 
			  CASE WHEN P.Cod_Libro IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Cod_Libro,'''','')+''',' END +
			  CASE WHEN P.Cod_TipoComprobante IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Cod_TipoComprobante,'''','')+''',' END +
			  CASE WHEN P.Serie IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Serie,'''','')+''',' END +
			  CASE WHEN P.Numero IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Numero,'''','')+''',' END +	
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
				 SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_CAJ_COMPROBANTE_LOG_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_CAJ_COMPROBANTE_PAGO_I '+ 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script='PALERPlink.'+@NombreBD+'.dbo.USP_CAJ_COMPROBANTE_RELACION_I '+ 
			  CASE WHEN P.Cod_Libro IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Cod_Libro,'''','')+''',' END +
			  CASE WHEN P.Cod_TipoComprobante IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Cod_TipoComprobante,'''','')+''',' END +
			  CASE WHEN P.Serie IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Serie,'''','')+''',' END +
			  CASE WHEN P.Numero IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Numero,'''','')+''',' END +	
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_CAJ_CONCEPTO_I ' + 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
				 SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_CAJ_FORMA_PAGO_I '+ 
				 CASE WHEN P.Cod_Libro IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Cod_Libro,'''','')+''',' END +
				 CASE WHEN P.Cod_TipoComprobante IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Cod_TipoComprobante,'''','')+''',' END +
				 CASE WHEN P.Serie IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Serie,'''','')+''',' END +
				 CASE WHEN P.Numero IS NULL THEN 'NULL,' ELSE ''''+REPLACE(P.Numero,'''','')+''',' END +	
...
BEGIN
	...
	-- 		--Si esta habilitada la exportacion para almacenar en la tabla de
	-- 		--exportaciones
	-- 		  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.CAJ_GUIA_REMISION_I '+ 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT  @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_CAJ_MEDICION_VC_I ' + 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_CAJ_TIPOCAMBIO_I ' +
...
BEGIN
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script='PALERPlink.'+@NombreBD+'.dbo.USP_PAR_COLUMNA_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			SELECT @Script='PALERPlink.'+@NombreBD+'.dbo.USP_PAR_FILA_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			SELECT  @Script='PALERPlink.'+@NombreBD+'.dbo.USP_PAR_TABLA_I ' +
			    CASE WHEN  i.Cod_Tabla IS NULL THEN 'NULL,' ELSE ''''+REPLACE(i.Cod_tabla,'''','')+''',' END +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script='PALERPlink.'+@NombreBD+'.dbo.USP_PRI_ACTIVIDADES_ECONOMICAS_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_AREAS_I ' +
			  ''''+REPLACE(pa.Cod_Area,'''','')+''','+
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_CLIENTE_CONTACTO_I ' +
			  CASE WHEN pcp.Cod_TipoDocumento IS NULL THEN 'NULL,' ELSE ''''+REPLACE(pcp.Cod_TipoDocumento,'''','') +''','END+
			  CASE WHEN pcp.Nro_Documento IS NULL THEN 'NULL,' ELSE ''''+REPLACE(pcp.Nro_Documento,'''','') +''','END+
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_CLIENTE_CUENTABANCARIA_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script='PALERPlink.'+@NombreBD+'.dbo.USP_PRI_CLIENTE_PRODUCTO_I '+
			  CASE WHEN CP.Cod_TipoDocumento IS NULL THEN 'NULL,' ELSE ''''+ REPLACE(CP.Cod_TipoDocumento,'''','')+''','END+
			  CASE WHEN CP.Nro_Documento IS NULL THEN 'NULL,' ELSE ''''+ REPLACE(CP.Nro_Documento,'''','')+''','END+ 
			  CASE WHEN P.Cod_Producto IS NULL THEN 'NULL,' ELSE ''''+ REPLACE(P.Cod_Producto,'''','')+''','END+ 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_CLIENTE_VEHICULOS_I '+
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_CLIENTE_VISITAS_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script='PALERPlink.'+@NombreBD+'.dbo.USP_PRI_CUENTA_CONTABLE_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
				 SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_DESCUENTOS_I ' +
				 CASE WHEN  pcp.Cod_TipoDocumento IS NULL  THEN 'NULL,'    ELSE ''''+ REPLACE(pcp.Cod_TipoDocumento,'''','')+''','END+
				 CASE WHEN  pcp.Nro_Documento IS NULL  THEN 'NULL,'    ELSE ''''+ REPLACE(pcp.Nro_Documento,'''','')+''','END+
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_EMPRESA_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script = 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_LICITACIONES_I ' + 
			  CASE WHEN CP.Cod_TipoDocumento IS NULL THEN 'NULL,' ELSE ''''+ REPLACE(CP.Cod_TipoDocumento,'''','')+''','END+
			  CASE WHEN CP.Nro_Documento IS NULL THEN 'NULL,' ELSE ''''+ REPLACE(CP.Nro_Documento,'''','')+''','END+ 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_LICITACIONES_D_I ' + 
			  CASE WHEN CP.Cod_TipoDocumento IS NULL THEN 'NULL,' ELSE ''''+ REPLACE(CP.Cod_TipoDocumento,'''','')+''','END+
			  CASE WHEN CP.Nro_Documento IS NULL THEN 'NULL,' ELSE ''''+ REPLACE(CP.Nro_Documento,'''','')+''','END+
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			 SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_LICITACIONES_M_I '+ 
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script = 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_MENSAJES_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script = 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_PADRONES_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
			  SELECT @Script = 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_PERSONAL_I ' +
   ...
   --Acciones
	...
	BEGIN
	    ...
	    BEGIN
			--Si esta habilitada la exportacion para almacenar en la tabla de
			--exportaciones
				 SELECT @Script= 'PALERPlink.'+@NombreBD+'.dbo.USP_PRI_PERSONAL_PARENTESCO_I ' +
