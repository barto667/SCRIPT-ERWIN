...
-- eliminar un comprobante en nuestro servidor remoto
IF EXISTS (SELECT name FROM sysobjects WHERE name = 'USP_CAJ_COMPROBANTE_PAGO_D' AND type = 'P')
	DROP PROCEDURE USP_CAJ_COMPROBANTE_PAGO_D
go
CREATE PROCEDURE USP_CAJ_COMPROBANTE_PAGO_D 
	@Cod_Libro	varchar(2), 
	@Cod_TipoComprobante	varchar(5), 
	@Serie	varchar(5), 
	@Numero	varchar(30), 
...
BEGIN
    ...
    BEGIN
	   EXEC USP_CAJ_COMPROBANTE_PAGO_E @Id_ComprobantePago,@Cod_Usuario,@Motivo
    END
END
go
...
-- --Solo para el servidor principal
-- --------------------------------------------------------------------------------------------------------------
...
-- 	@Total	numeric(38,2), 
-- 	@Forma_Pago	varchar(512),
-- 	@Moneda varchar(512),
-- 	@Cod_EstadoComprobante varchar(32),
-- 	@Cod_Usuario Varchar(32)
-- WITH ENCRYPTION
-- AS
-- BEGIN
-- 	DECLARE @iu_ComprobantePago UNIQUEIDENTIFIER;
-- IF NOT EXISTS (SELECT * FROM CAJ_COMPROBANTE_PAGO WHERE  (Cod_TipoComprobante = @Cod_TipoComprobante and Serie = @Serie and Numero = @Numero 
...
-- 			Cod_UsuarioAct= @Cod_Usuario,
-- 			Fecha_Act=GETDATE()
-- 		WHERE (Cod_TipoComprobante = @Cod_TipoComprobante and Serie = @Serie and Numero = @Numero and RUC_Emisor = @RUC_Emisor)	
-- 	END
-- END
-- GO
-- go

--Auditoria SQL server, solo usado para casos muy especiales
-- USE [master]
-- GO

-- /****** Object:  Audit [Auditoria]    Script Date: 20/04/2018 09:17:03 a.m. ******/
-- CREATE SERVER AUDIT [Auditoria]
-- TO FILE 
-- (	FILEPATH = N'H:\AUDITORIA\'
-- 	,MAXSIZE = 0 MB
...
-- (	QUEUE_DELAY = 1000
-- 	,ON_FAILURE = CONTINUE
-- 	,AUDIT_GUID = '748d1ebf-4e9f-4365-a3a1-55fe2e32e151'
-- )
-- ALTER SERVER AUDIT [Auditoria] WITH (STATE = ON)
-- GO

-- CREATE SERVER AUDIT SPECIFICATION [ServerAuditSpecification-20180417-130035]
-- FOR SERVER AUDIT [Auditoria]
-- ADD (DATABASE_OPERATION_GROUP),
-- ADD (SERVER_OPERATION_GROUP)
-- WITH (STATE = ON)
-- GO
-- CREATE DATABASE AUDIT SPECIFICATION [DatabaseAuditSpecification-20180417-130216]
-- FOR SERVER AUDIT [Auditoria]
-- ADD (UPDATE ON OBJECT::[dbo].[CAJ_COMPROBANTE_PAGO] BY [dbo])
-- WITH (STATE = ON)
-- GO