SET DATEFORMAT dmy
--Verificar que no existan comprobante electronicos afectando al mismo empresa
SELECT ccp.id_ComprobantePago,ccp.Cod_Caja,ccp.Cod_TipoComprobante,ccp.Serie,ccp.Numero,ccp.Total 
FROM dbo.CAJ_COMPROBANTE_PAGO ccp WHERE ccp.Cod_TipoComprobante IN ('BE','FE','NCE','NDE') AND ccp.Cod_Libro=14 
AND ccp.Doc_Cliente=(SELECT TOP 1 pe.RUC FROM dbo.PRI_EMPRESA pe)
AND ccp.Cod_EstadoComprobante NOT IN ('ACS','ENB','ENF','ENE','FIN','REC') 
AND ccp.Flag_Anulado=0

--VErifica si algun documento no fue enviado en el plazo de 7 dias o para adelante

SELECT ccp.id_ComprobantePago,ccp.Cod_Caja,ccp.Cod_TipoComprobante,ccp.Serie,ccp.Numero,ccp.Doc_Cliente,ccp.Nom_Cliente,ccp.Total,ccp.FechaEmision 
...
OR DATEDIFF (DAY,convert(datetime,CONVERT(VARCHAR(10),ccp.FechaEmision,103)),convert(datetime,CONVERT(VARCHAR(10),GETDATE(),103)))<-1)


--Verificar que no existan campos null en cod_IGV o tipo
SELECT distinct ccp.id_ComprobantePago,ccp.Cod_Caja,ccd.id_Detalle,ccp.Cod_TipoComprobante,ccp.Serie,ccp.Numero,ccd.Cod_TipoIGV,ccd.Tipo,ccp.FechaEmision 
FROM dbo.CAJ_COMPROBANTE_PAGO ccp INNER JOIN dbo.CAJ_COMPROBANTE_D ccd ON ccp.id_ComprobantePago = ccd.id_ComprobantePago
WHERE (ccd.Cod_TipoIGV IS NULL OR REPLACE(ccd.Cod_TipoIGV,' ','')='') AND ccp.Cod_Libro=14  
...
--Verifica los tipos de documento para facturas
select id_ComprobantePago,ccp.Cod_Caja,Cod_TipoComprobante,Serie,Numero,ccp.Cod_TipoOperacion,Cod_TipoDoc,Doc_Cliente,Nom_Cliente,Direccion_Cliente,FechaEmision
 from CAJ_COMPROBANTE_PAGO ccp
where ccp.Cod_TipoComprobante IN ('FE','NCE','NDE') AND ccp.Serie LIKE 'F%' and (ccp.Cod_TipoDoc<>6 or LEN(ccp.Doc_Cliente)<>11 
or LEN(REPLACE(ccp.Nom_Cliente,' ',''))=0  )
and ccp.Cod_Libro=14 and ccp.Flag_Anulado=0
...
select ccp.id_ComprobantePago,ccd.id_Detalle,ccp.Cod_Caja,ccp.Cod_TipoComprobante,ccp.Serie,ccp.Numero,ccp.Doc_Cliente,ccp.Nom_Cliente,ccp.Total,ccp.FechaEmision 
from CAJ_COMPROBANTE_PAGO ccp inner join CAJ_COMPROBANTE_D ccd on ccp.id_ComprobantePago=ccd.id_ComprobantePago
...
SELECT  * FROM dbo.CAJ_COMPROBANTE_PAGO ccp INNER JOIN dbo.CAJ_COMPROBANTE_D ccd ON ccp.id_ComprobantePago = ccd.id_ComprobantePago
INNER JOIN (
select ccd.id_ComprobantePago, count(ccd.id_ComprobantePago) Numero,SUM(ccd.Sub_Total) AS aTotal
from CAJ_COMPROBANTE_D ccd INNER JOIN dbo.CAJ_COMPROBANTE_PAGO ccp ON ccd.id_ComprobantePago = ccp.id_ComprobantePago
WHERE  ccp.Cod_TipoComprobante IN ('FE','BE','NCE','NDE')  
and ccp.Cod_Libro=14 and ccp.Flag_Anulado=0
--AND ccp.Cod_EstadoComprobante NOT IN ('ACS','ENB','ENF','ENE','FIN','REC') 
--AND ccp.Flag_Anulado=0
group by ccd.id_ComprobantePago
having count(ccd.id_ComprobantePago) > 1) cct ON cct.id_ComprobantePago<>ccp.id_ComprobantePago
WHERE 
ccp.Cod_TipoComprobante IN ('FE','BE','NCE','NDE')  
and ccp.Cod_Libro=14 
AND ccp.Cod_EstadoComprobante NOT IN ('ACS','ENB','ENF','ENE','FIN','REC') 
AND ccp.Flag_Anulado=0
--AND cct.Numero=1 AND cct.aTotal=0
AND (((ccd.Cod_TipoIGV=10 AND (ccd.Sub_Total=0 OR ccd.IGV=0)) OR (ccd.Cod_TipoIGV=13 OR (ccd.Sub_Total<>0 OR ccd.IGV<>0))))
...
AND ccp.Flag_Anulado=0
AND cct.Numero=1 AND cct.aTotal=0
AND (((ccd.Cod_TipoIGV=20 AND (ccd.Sub_Total=0 OR ccd.IGV<>0)) OR (ccd.Cod_TipoIGV=21 OR (ccd.Sub_Total<>0 OR ccd.IGV<>0))))
...
SELECT * FROM dbo.CAJ_COMPROBANTE_PAGO ccp WHERE ccp.id_ComprobantePago=5427
       ...
       AND (ccp.Doc_Cliente IS NULL
            OR ccp.Nom_Cliente IS NULL
            OR LEN(REPLACE(ccp.Nom_Cliente, ' ', '')) < 5);
