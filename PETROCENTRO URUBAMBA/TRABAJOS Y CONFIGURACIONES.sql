...
--3. PROCEDIMIENTO DE ACTUALIZACION
--Ejecutar todo en la bd destino
--El nombre de la BD destino y el de la nube deben ser iguales

--Ejecutar esto en la BD destino 
...
AND Numero=@NumeroRef AND Id_Cliente=@Id_Cliente)
...
BEGIN
BEGIN TRY   
	SELECT *  into #Comprobantes FROM (
	SELECT TOP 1000 
    'PALERPcomprobantes.PALERPcomprobantes.dbo.USP_CAJ_COMPROBANTE_PAGO_G ' +
				CASE WHEN  pe.RUC IS NULL THEN 'NULL,' ELSE ''''+pe.RUC+''','END+
				CASE WHEN ccp.Cod_TipoComprobante IS NULL THEN 'NULL,' ELSE ''''+ccp.Cod_TipoComprobante+''','END+
				CASE WHEN ccp.Serie IS NULL THEN 'NULL,' ELSE ''''+ccp.Serie+''','END+
				CASE WHEN ccp.Numero IS NULL THEN 'NULL,' ELSE ''''+ccp.Numero+''','END+
				CASE WHEN ccp.Cod_TipoDoc IS NULL THEN 'NULL,' ELSE ''''+ccp.Cod_TipoDoc+''','END+
				CASE WHEN ccp.Doc_Cliente IS NULL THEN 'NULL,' ELSE ''''+ccp.Doc_Cliente+''','END+
				...
				AS ScriptComprobantes, ccp.FechaEmision,ccp.id_ComprobantePago
    ...
    WHERE ((ccp.Cod_TipoComprobante IN('FE', 'BE', 'NCE', 'NDE')
	    AND ccp.Cod_EstadoComprobante = 'FIN') OR (ccp.Cod_TipoComprobante IN('TKB','TKF'))
	...
	DECLARE  RecorrerScriptComprobantes CURSOR FOR SELECT c.ScriptComprobantes,c.id_ComprobantePago FROM #Comprobantes c ORDER BY c.FechaEmision
	OPEN RecorrerScriptComprobantes
	FETCH NEXT FROM RecorrerScriptComprobantes 
	INTO @Linea,@IdComprobante
	WHILE @@FETCH_STATUS = 0
	BEGIN   
		print @Linea
		--DECLARE @Resultado varchar(max) ='master..xp_cmdshell ' +''''+ REPLACE(REPLACE('echo LINEA A EJECUTAR: '+@Linea+'>> C:\APLICACIONES\LOG\log_exportacion.txt','''',' '),'|',' ')+''''
		...
		BEGIN CATCH
			...
			,ERROR_LINE() AS ErrorLine  
		 	,ERROR_MESSAGE() AS ErrorMessage;  
		 	SET @Resultado  = 'master..xp_cmdshell ' +''''+ REPLACE(REPLACE('echo ERROR DURANTE LA EJECUCION DEL USP : '+ERROR_MESSAGE()+CONVERT(varchar(max),GETDATE(),121)+'>> C:\APLICACIONES\TEMP\log_Comprobantes_Subidos.txt','''',' '),'|',' ')+''''
			EXECUTE(@Resultado)
		END CATCH
	FETCH NEXT FROM RecorrerScriptComprobantes 
	INTO @Linea,@IdComprobante
	...
	SET @Resultado  ='master..xp_cmdshell ' +''''+ REPLACE(REPLACE('echo EXITO EN LA SUBIDA DE DATOS : '+CONVERT(varchar(max),GETDATE(),121)+'>> C:\APLICACIONES\TEMP\log_Comprobantes_Subidos.txt','''',' '),'|',' ')+''''
	EXECUTE(@Resultado)
...
BEGIN CATCH  
		SELECT   
			...
			,ERROR_LINE() AS ErrorLine  
			,ERROR_MESSAGE() AS ErrorMessage;  
		SET @Resultado  = 'master..xp_cmdshell ' +''''+ REPLACE(REPLACE('echo ERROR DURANTE LA EJECUCION: '+ERROR_MESSAGE()+' No se puede realizar la subida de comprobantes: '+CONVERT(varchar(max),GETDATE(),121)+'>> C:\APLICACIONES\TEMP\log_Comprobantes_Subidos.txt','''',' '),'|',' ')+''''
		EXECUTE(@Resultado)
END CATCH;  
...
--ANTA
 SELECT ccp.Cod_TipoComprobante, COUNT(*) AS Total FROM dbo.CAJ_COMPROBANTE_PAGO ccp 
 WHERE ccp.Cod_Caja IN ('301','302','303') AND ccp.Cod_Libro=14 AND (ccp.Serie LIKE 'B%' OR ccp.Serie LIKE 'F%')
 GROUP BY  ccp.Cod_TipoComprobante

 --PUCYURA
  SELECT ccp.Cod_TipoComprobante, COUNT(*) AS Total FROM dbo.CAJ_COMPROBANTE_PAGO ccp 
 WHERE ccp.Cod_Caja IN ('201','202') AND ccp.Cod_Libro=14 AND (ccp.Serie LIKE 'B%' OR ccp.Serie LIKE 'F%')
 GROUP BY  ccp.Cod_TipoComprobante


 --YANAHUARA
   SELECT ccp.Cod_TipoComprobante, COUNT(*) AS Total FROM dbo.CAJ_COMPROBANTE_PAGO ccp 
 WHERE ccp.Cod_Caja IN ('501','502') AND ccp.Cod_Libro=14 AND (ccp.Serie LIKE 'B%' OR ccp.Serie LIKE 'F%')
 GROUP BY  ccp.Cod_TipoComprobante

 --URUBAMBA
    SELECT ccp.Cod_TipoComprobante, COUNT(*) AS Total FROM dbo.CAJ_COMPROBANTE_PAGO ccp 
 WHERE ccp.Cod_Caja IN ('401','402') AND ccp.Cod_Libro=14 AND (ccp.Serie LIKE 'B%' OR ccp.Serie LIKE 'F%')
 GROUP BY  ccp.Cod_TipoComprobante

  --ARCOPATA
    SELECT ccp.Cod_TipoComprobante, COUNT(*) AS Total FROM dbo.CAJ_COMPROBANTE_PAGO ccp 
 WHERE ccp.Cod_Caja IN ('601','602') AND ccp.Cod_Libro=14 AND (ccp.Serie LIKE 'B%' OR ccp.Serie LIKE 'F%')
 GROUP BY  ccp.Cod_TipoComprobante

  --POROY
    SELECT ccp.Cod_TipoComprobante, COUNT(*) AS Total FROM dbo.CAJ_COMPROBANTE_PAGO ccp 
 WHERE ccp.Cod_Caja IN ('101','102','103','104') AND ccp.Cod_Libro=14 AND (ccp.Serie LIKE 'B%' OR ccp.Serie LIKE 'F%')
 GROUP BY  ccp.Cod_TipoComprobante



  SELECT Serie,Count(*) from CAJ_COMPROBANTE_PAGO ccp 
 WHERE ccp.Cod_Caja IN ('101','102') AND ccp.Cod_TipoComprobante='FE'   AND ccp.Cod_Libro=14 AND  ccp.Serie LIKE 'F%'
 GROUP BY Serie


--Especiales para URUBAMBA
--V2
...
UPDATE dbo.CAJ_COMPROBANTE_D SET 
	...
	dbo.CAJ_COMPROBANTE_D.Cod_UsuarioAct='MIGRACION',
...
BEGIN  
   -- Actualizar los totaltes y el igv
	update CAJ_COMPROBANTE_PAGO set Total = @CTotal, Impuesto = @IGV,Cod_UsuarioReg='MIGRACION',Fecha_Act=GETDATE()
	where id_ComprobantePago =   @id_ComprobantePago        
  -- actualizar las formas de pago
  update CAJ_FORMA_PAGO SET Monto = @CTotal,Cod_UsuarioReg='MIGRACION',Fecha_Act=GETDATE()
...
BEGIN   
	...
	UPDATE CAJ_COMPROBANTE_PAGO SET NUMERO = RIGHT('00000000'+CONVERT( varchar(38), @numero,0), 8),Cod_UsuarioAct='MIGRACION',Fecha_Act=GETDATE()
	WHERE id_ComprobantePago = @id_ComprobantePago 

	UPDATE dbo.CAJ_COMPROBANTE_D SET  Cod_UsuarioAct='MIGRACION',Fecha_Act=GETDATE()
	WHERE id_ComprobantePago = @id_ComprobantePago

	UPDATE dbo.CAJ_FORMA_PAGO SET  Cod_UsuarioAct='MIGRACION',Fecha_Act=GETDATE()
	WHERE id_ComprobantePago = @id_ComprobantePago
...
CREATE FUNCTION FUN_Caj_Comprobante_Pago_ObtenerTotal
(
     @Cod_Libro varchar(max),
	@Cod_TipoComprobante varchar(max),
	@Serie varchar(max),
	@Numero varchar(max)
) 
...
BEGIN
	...
	SET @v_ret =  (SELECT TOP 1 ccp.Total FROM dbo.CAJ_COMPROBANTE_PAGO ccp WHERE ccp.Cod_Libro=@Cod_Libro AND ccp.Cod_TipoComprobante=@Cod_TipoComprobante
...
--DECLARE @Cod_Caja varchar(max)='101'
DECLARE @Id_ComprobanteL int
DECLARE @Cod_TipoComprobanteL varchar(max)
DECLARE @Cod_libroL varchar(max)
DECLARE @SerieL varchar(max)
DECLARE @NumeroL varchar(max)
DECLARE @TotalL numeric (38,2)
DECLARE @TotalS numeric (38,2)
declare @FechaActual datetime = GETDATE()

set dateformat dmy

DECLARE cursorfila CURSOR LOCAL FOR
select distinct id_ComprobantePago,Cod_Libro, Cod_TipoComprobante,Serie,Numero,Total from CAJ_COMPROBANTE_PAGO where Cod_TipoComprobante in ('FE','BE','NCE','NDE')
and Cod_Libro=14 and (Serie like 'B%' or Serie like 'F%') 
and FechaEmision<= CONVERT(DATETIME, CONVERT(VARCHAR(10), @FechaActual, 103))
--and DATEDIFF(DAY, CONVERT(DATETIME, CONVERT(VARCHAR(10), FechaEmision, 103)), CONVERT(DATETIME, CONVERT(VARCHAR(10), @FechaActual, 103))) > 1
--and Cod_Caja=@Cod_Caja 
order by Cod_TipoComprobante,Serie,Numero 
OPEN cursorfila

FETCH NEXT FROM cursorfila INTO @Id_ComprobanteL,@Cod_libroL, @Cod_TipoComprobanteL,@SerieL,@NumeroL,@TotalL

WHILE @@FETCH_STATUS = 0
BEGIN
	EXEC @TotalS= [PALERPlink] .PALERPurubamba.dbo.FUN_Caj_Comprobante_Pago_ObtenerTotal @Cod_libroL,@Cod_TipoComprobanteL,@SerieL,@NumeroL
	IF @TotalS is null
	BEGIN
		PRINT 'No existe el comprobante => '+@Cod_TipoComprobanteL+':'+@SerieL+'-'+@NumeroL
		INSERT INTO #tempTablaResultado values 
		(
			'1',
			'No existe el comprobante',
			@Id_ComprobanteL,
			@Cod_TipoComprobanteL+':'+@SerieL+'-'+@NumeroL
		)
	END
	ELSE
	BEGIN
		IF @TotalL <> @TotalS
		BEGIN
			PRINT 'Los totales no coinciden => '+@Cod_TipoComprobanteL+':'+@SerieL+'-'+@NumeroL
			INSERT INTO #tempTablaResultado values 
			(
				'2',
		...
		--ELSE
		--BEGIN
		--	PRINT 'El comprobante existe y coinciden montos => '+@Cod_TipoComprobanteL+':'+@SerieL+'-'+@NumeroL
		--	INSERT INTO #tempTablaResultado values 
		--	(
		--		'3',
		--		'Coinciden',
...
select * from #tempTablaResultado



 UPDATE dbo.PRI_CLIENTE_PROVEEDOR
 SET dbo.PRI_CLIENTE_PROVEEDOR.Fecha_Act=GETDATE() WHERE dbo.PRI_CLIENTE_PROVEEDOR.Nro_Documento IN 
 (SELECT DISTINCT ccp.Doc_Cliente FROM dbo.CAJ_COMPROBANTE_PAGO ccp WHERE ccp.Cod_TipoComprobante='BE' AND ccp.Serie='B001' AND ccp.Cod_Libro=14 AND ccp.Numero>=2550 AND ccp.Numero<=100000)

UPDATE dbo.CAJ_COMPROBANTE_PAGO
SET
    dbo.CAJ_COMPROBANTE_PAGO.Fecha_Act=GETDATE() WHERE dbo.CAJ_COMPROBANTE_PAGO.Cod_TipoComprobante='BE' AND dbo.CAJ_COMPROBANTE_PAGO.Serie='B001' AND dbo.CAJ_COMPROBANTE_PAGO.Cod_Libro=14
    AND dbo.CAJ_COMPROBANTE_PAGO.Numero>=2550 AND dbo.CAJ_COMPROBANTE_PAGO.Numero<=100000

UPDATE dbo.CAJ_COMPROBANTE_D
SET dbo.CAJ_COMPROBANTE_D.Fecha_Act=GETDATE() WHERE dbo.CAJ_COMPROBANTE_D.id_ComprobantePago IN 
(SELECT ccp.id_ComprobantePago FROM dbo.CAJ_COMPROBANTE_PAGO ccp WHERE ccp.Cod_TipoComprobante='BE' AND ccp.Serie='B001' AND ccp.Cod_Libro=14 AND ccp.Numero>=2550 AND ccp.Numero<=100000)


 UPDATE dbo.CAJ_FORMA_PAGO
SET dbo.CAJ_FORMA_PAGO.Fecha_Act=GETDATE() WHERE dbo.CAJ_FORMA_PAGO.id_ComprobantePago IN 
(SELECT ccp.id_ComprobantePago FROM dbo.CAJ_COMPROBANTE_PAGO ccp WHERE ccp.Cod_TipoComprobante='BE' AND ccp.Serie='B001' AND ccp.Cod_Libro=14 AND ccp.Numero>=2550 AND ccp.Numero<=100000)

UPDATE dbo.CAJ_COMPROBANTE_RELACION
SET dbo.CAJ_COMPROBANTE_RELACION.Fecha_Act=GETDATE() WHERE dbo.CAJ_COMPROBANTE_RELACION.id_ComprobantePago IN 
(SELECT ccp.id_ComprobantePago FROM dbo.CAJ_COMPROBANTE_PAGO ccp WHERE ccp.Cod_TipoComprobante='BE' AND ccp.Serie='B001' AND ccp.Cod_Libro=14 AND ccp.Numero>=2550 AND ccp.Numero<=100000)




--Scripts para resubir datos (beta)

--Verifica si un job determinado esta en ejcucion, si este no esta en ejecucion no devuelve nada
--caso contrario devuelve una fila con su estado
IF EXISTS (SELECT name FROM sysobjects WHERE name = 'USP_VerificarEstadoTrabajo' AND type = 'P')
DROP PROCEDURE USP_VerificarEstadoTrabajo
go
CREATE PROCEDURE USP_VerificarEstadoTrabajo
@NombreTarea varchar(max)
WITH ENCRYPTION
AS
BEGIN
	SELECT *
		  ...
		  WHERE J.name=@NombreTarea 
		  AND A.run_requested_date IS NOT NULL 
		  AND A.stop_execution_date IS NULL
END


--Se obtiene la siguiente ejecucion del paso de la tarea
USE msdb;
EXEC dbo.sp_help_job
    @job_name = N'COPIA DE SEGURIDAD_SERVICENTRO',
